...
Large language models based on the Transformer architecture~\cite{vaswani2017attention} have become ubiquitous, powering applications from search engines to personal assistants. While most research focuses on GPU-based inference, a growing class of deployment scenarios: edge devices, mobile phones, embedded systems must run inference on CPU-only hardware. On such platforms, exploiting multicore parallelism efficiently is essential to achieve acceptable latency.

Transformer inference is dominated by general matrix multiplication (GEMM), which accounts for 70--90\% of total compute time~\cite{dice2021transformercpu}. Parallelizing these operations across CPU cores is straightforward; parallelizing them \emph{well} is not. Modern CPUs, particularly mobile and embedded SoCs, feature heterogeneous core configurations, different clock speeds, cache sizes, and memory bandwidths, that render uniform load distribution ineffective. The Qualcomm Snapdragon used in this work, for instance, combines a high-frequency Cortex-X3 prime core, mid-range performance cores, and energy-efficient cores sharing L2 cache, all behind a single shared 8\,MB L3.

Thread scheduling policy is a first-order factor in this setting. Two classical paradigms exist: \emph{work sharing}, where the scheduler proactively distributes tasks to all threads at spawn time, and \emph{work stealing}~\cite{blumofe1999workstealing}, where idle threads autonomously pull tasks from busy peers. Work sharing, as mplemented via OpenMP, is already present in the \texttt{StrasGPT} inference engine. Whether work stealing can improve over it on a heterogeneous ARM SoC, where synchronization costs, core asymmetry, and cache topology interact, is an open empirical question.

This paper addresses that question through three contributions:
\begin{enumerate}
    \item A custom work-stealing micro-scheduler implemented in C, integrated into \texttt{StrasGPT} via a Chase--Lev deque with spin-based barriers and false-sharing mitigation, activatable at runtime without recompilation.
    \item An empirical comparison of work stealing versus OpenMP work sharing across thread counts, core affinity configurations, and two models (Llama-3.2-1B and Qwen3-0.6B), measuring prefill and decode throughput.
    \item An analysis of how the heterogeneous big.LITTLE topology shapes scheduling efficiency, identifying configurations where work stealing provides consistent gains and those where it regresses.
\end{enumerate}

\section{Background}
\label{sec:background}

This section introduces the key concepts required to understand the rest of thisreport. It covers the Transformer architecture and how inference works, the two scheduling paradigms compared in this work, and the thread pinning mechanisms used to control core affinity on heterogeneous hardware. The central question addressed throughout is: \emph{what is the best thread scheduling strategy for Transformer inference on a heterogeneous multicore CPU?}

\subsection{Transformer Inference on CPU}

A \textbf{Transformer}~\cite{vaswani2017attention} is a neural network architecture based on the \emph{self-attention} mechanism. It processes sequences of tokens through a stack of \emph{decoder layers}, each containing a multi-head attention (MHA) block and a feed-forward network (FFN). Inference proceeds in two phases:

\begin{itemize}
    \item \textbf{Prefill}: the full input prompt (a sequence of $T$ tokens) is processed in parallel. Computations are data-parallel across tokens and attention heads, making this phase amenable to aggressive parallelization.
    \item \textbf{Decode}: tokens are generated one at a time in an autoregressive loop. Each forward pass processes a single new token while attending over a \emph{KV-cache} (key-value cache) that accumulates all previously generated key and value projections. This phase is memory-bandwidth-bound rather than compute-bound.
\end{itemize}

The dominant operation in both phases is \textbf{GEMM} (General Matrix Multiplication), which accounts for 70--90\% of total inference time~\cite{dice2021transformercpu}. All other operations: softmax, layer normalization, residual additions, are negligible by comparison. Parallelizing GEMM efficiently across CPU cores is therefore the primary performance lever.

\textbf{StrasGPT} is the inference engine used in this work. It is a lightweight C implementation of a Transformer decoder, targeting shared-memory multicore CPUs including heterogeneous ARM \textbf{big.LITTLE} SoCs (System on a Chip). Its parallelism is expressed via OpenMP, and this report extends it with a custom work-stealing scheduler.

\subsection{Thread Pinning and Core Affinity}

On a multicore CPU, the OS scheduler may migrate a thread between cores during execution. Such migration is harmful for inference because it destroys L1/L2 cache residency and may move a thread to a core with a different frequency, disrupting the load balance assumptions of the work-stealing initializer.

\textbf{Thread pinning} (also called \emph{core affinity}) binds each thread to a specific physical core using OS primitives. On Linux, this is done via the \texttt{taskset} command or the \texttt{sched\_setaffinity} syscall, which sets the \textbf{CPU affinity mask} of a process or thread, a bitmask indicating on which cores it is allowed to run.

In this work, affinity is controlled at the process level via \texttt{taskset} before launching \texttt{StrasGPT}, so that OpenMP thread $i$ is pinned to physical core P\#$i$. Four affinity configurations are evaluated (detailed in Section~\ref{sec:eval}):

\begin{itemize}
    \item \textbf{cpu\_7}: single prime core (Cortex-X3 only). Sequential baseline.
    \item \textbf{cpu\_0\_2}: efficiency cores only (3$\times$ Cortex-A510, 2.0~GHz). Homogeneous low-frequency cluster.
    \item \textbf{cpu\_3\_6}: performance cores only (2$\times$ A710 + 2$\times$ A715, 2.8~GHz). Homogeneous high-frequency cluster.
    \item \textbf{cpu\_0\_7}: all 8 cores. Maximally heterogeneous: A510 (2.0~GHz) through X3 (3.36~GHz), a 1.68$\times$ frequency ratio.
\end{itemize}

The choice of affinity mask has a first-order effect on scheduling performance: it determines whether the core set is homogeneous (making load balance trivial) or heterogeneous (making it a hard problem that motivates work stealing).

\subsection{Problem Statement}

This report investigates the following question: \emph{for Transformer inference on a heterogeneous ARM big.LITTLE CPU, does a custom work-stealing scheduler outperform OpenMP work sharing, and under which conditions?}

To answer it, a Chase--Lev work-stealing scheduler with heterogeneity-aware initial partitioning is implemented in C and integrated into StrasGPT. It is evaluated against OpenMP \texttt{schedule(dynamic)} across four affinity configurations, two models (Llama-3.2-1B and Qwen3-0.6B), two sequence lengths, and thread counts from 1 to 8. Performance is measured in tokens per second (TPS) for both prefill and decode phases.

\section{State of the art}
    \subsection{Thread Scheduling in Parallel Computing}

    Thread scheduling has been a fundamental problem in parallel computing since several decades: given a set of computations and a fixed number of CPU cores, the scheduler has to decide which thread runs on which core at which time. As of today, where multicore architectures have become the norm, these decisions have a critical impact on load balance, cache utilization, memory locality, and synchronization overhead. To address this problem, two main families of scheduling policies have been found to be effective: \emph{work sharing} and \emph{work stealing}.
...
\section{Conclusion}
This work investigated whether a custom work-stealing scheduler can improve transformer inference throughput over OpenMP work sharing on a heterogeneous ARM big.LITTLE SoC. Three findings stand out.

\textbf{Homogeneous clusters reward work stealing.} On both the efficiency cluster (cpu\_0\_2, 3$\times$ A510) and the performance cluster (cpu\_3\_6, A710/A715), the work-stealing scheduler delivers consistent decode speedups of $+5$--$13\%$ across thread counts and both models. The gains are most stable on the performance cluster, where larger private L2 caches reduce the relative cost of stealing.

\textbf{Mixed-frequency configurations expose a fundamental limitation.} At $N = 4$--$6$ threads on the full platform (cpu\_0\_7), where A510 and A710/A715 cores coexist, the scheduler regresses by up to $-17\%$ on decode throughput. This pathology stems from two compounding issues: the range-based task decomposition is too coarse to rebalance iteration-level imbalance, and the frequency-proportional weight heuristic does not accurately reflect actual throughput under memory pressure. When the prime X3 core is added at $N = 7$--$8$, the load imbalance grows severe enough that work stealing recovers strongly ($+19$--$29\%$), as the prime core's surplus capacity more than compensates for the heuristic's imprecision.

\textbf{Static scheduling is catastrophic on heterogeneous hardware.} As a motivating baseline, the static OpenMP schedule degrades decode throughput dramatically relative to both dynamic and work-stealing alternatives, confirming that any fixed-partition strategy is fundamentally incompatible with asymmetric core frequencies.

The main open challenge is the mixed-frequency regime. Future work should explore finer task granularity (splitting ranges into individual iterations), topology-aware victim selection to prefer stealing from fast cores, and runtime-measured throughput estimates in place of static frequency ratios. A NUMA-aware extension along the lines of Al-Omairy et al.~\cite{alomairy2015numa} could further reduce the cost of cross-cluster steals.
